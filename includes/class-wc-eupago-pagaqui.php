...
* @since 1.1
*/
if ( !class_exists( 'WC_EuPago_Pagaqui_WebAtual' ) ) {
  class WC_EuPago_Pagaqui_WebAtual extends WC_Payment_Gateway {
    ...
    function init_form_fields() {
      $this->form_fields = array(
        'enabled' => array(
          ...
          'type' => 'checkbox',
          'label' => __( 'Enable Pagaqui (using euPago)', 'eupago-gateway-for-woocommerce'),
          'default' => 'no'
        ),
        'title' => array(
          ...
          'default' => __('Pagaqui', 'eupago-gateway-for-woocommerce')
        ),
        'description' => array(
          ...
          'default' => __('Pay at Pagaqui agent', 'eupago-gateway-for-woocommerce')
        ),
        'instructions' => array(
    ...
    public function thankyou_page( $order_id ) {
      ...
      if ( $payment_method == $this->id ) {
        ...
        wc_get_template( 'payment-instructions.php', array(
          ...
          'referencia' => get_post_meta($order_id, '_eupago_pagaqui_referencia', true),
    ...
    */
    function process_payment($order_id) {
      ...
      $eupagoPagaqui = $this->client->getReferenciaPQ( $order_id, $order_total );

      if ( $eupagoPagaqui->estado != 0 ) {
        $error_message = $eupagoPagaqui->resposta;
        wc_add_notice( __('Payment error:', 'eupago-gateway-for-woocommerce') . $error_message, 'error' );
        return;
      }

      update_post_meta ($order_id, '_eupago_pagaqui_referencia', $eupagoPagaqui->referencia);

      // Mark as on-hold
      $order->update_status('on-hold', __('Awaiting Pagaqui payment.', 'eupago-gateway-for-woocommerce'));
