...
* @since 2.0
*/
if ( !class_exists( 'WC_EuPago_PSC_WebAtual' ) ) {
  class WC_EuPago_PSC_WebAtual extends WC_Payment_Gateway {
    ...
    */
    public function __construct() {
      ...
      $this->icon = plugins_url('assets/images/psc_icon.png', dirname(__FILE__));
      $this->has_fields = false;
      $this->method_title = __('Paysafecash (EuPago)', 'eupago-gateway-for-woocommerce');
    ...
    function init_form_fields() {
      $this->form_fields = array(
        'enabled' => array(
          ...
          'type' => 'checkbox',
          'label' => __( 'Enable Paysafecash (using euPago)', 'eupago-gateway-for-woocommerce'),
          'default' => 'no'
        ),
        'title' => array(
          ...
          'default' => __('Paysafecash', 'eupago-gateway-for-woocommerce')
        ),
        'description' => array(
          ...
          'default' => __('Pay with cash (Paysafecash)', 'eupago-gateway-for-woocommerce')
        ),
        'logo_url' => array(
    ...
    public function thankyou_page( $order_id ) {
      ...
      if ( $payment_method == $this->id ) {
        ...
        wc_get_template( 'payment-instructions.php', array(
          ...
          'referencia' => get_post_meta($order_id, '_eupago_psc_referencia', true),
    ...
    */
    function process_payment($order_id) {
      ...
      $eupagoPSC = $this->client->pedidoPSC( $order, $order_total, $this->get_return_url( $order ), $lang, $this->get_comment_table( $order, $order_total ) );
      
      if ( $eupagoPSC->estado != 0 ) {
        $error_message = $eupagoPSC->resposta;
        wc_add_notice( __('Payment error:', 'eupago-gateway-for-woocommerce') . ' ' . $error_message, 'error' );
        return;
      }

      update_post_meta ($order_id, '_eupago_psc_referencia', $eupagoPSC->referencia);

      // Mark as on-hold
      $order->update_status('on-hold', __('Awaiting Paysafecash payment.', 'eupago-gateway-for-woocommerce'));
